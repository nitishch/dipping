function [output_image] = main_color(input_image_path, orows, ocols, epsilon, window_size)
    ...
    %% Adding seed
    output_image = double(zeros(orows, ocols, 3));
    ...
    padded_output = double(zeros(size(output_image,1) + window_size - 1,...
        size(output_image,2) + window_size - 1, 3));
    ...
    A = im2col(input_image(:,:,1), [window_size window_size], 'sliding');
    windows = zeros(size(A',1), size(A',2), 3);
    windows(:,:,1) = A';
    windows(:,:,2) = im2col(input_image(:,:,2), [window_size window_size], 'sliding')';
    windows(:,:,3) = im2col(input_image(:,:,3), [window_size window_size], 'sliding')';
    ...
    function [distance_vector] = distfun(z1byn, zmbyn)
        ...
        gaussian_repeated = repmat(gaussian, size(zmbyn, 1), 3); 
...
%          end
    ...
    function [o] = giveValue(probe_image)
        probe_red = probe_image(:, :, 1);
        probe_green = probe_image(:, :, 2);
        probe_blue = probe_image(:, :, 3);
        probe_image = [probe_red(:)', probe_green(:)', probe_blue(:)'];
        ...
        o = [temp(ceil((window_size * window_size)/2));...
            temp(ceil((3 * window_size * window_size)/2));...
            temp(ceil((5 * window_size * window_size)/2))];
    end
